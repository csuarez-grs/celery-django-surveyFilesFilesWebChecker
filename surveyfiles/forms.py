...
class DataExportForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['document', 'site_no', 'extract_input_values',
                  'utm_sr_name', 'scale_value',
                  'notify_surveyor', 'notify_pm', 'raise_invalid_errors', 'site_data_db',
    ...
    def clean(self):
        ...
        if site_data_db is not None:
            site_data_db = site_data_db.replace('R:', r'\\grs.com\DFS\JOBS')
            self.cleaned_data['site_data_db'] = site_data_db
        ...
        if document_name is not None and re.search(fortis_job_no_pattern, document_name):
            document_job_no = re.search(fortis_job_no_pattern, document_name).groups()[0]
            if site_data_db is not None and re.search(fortis_job_no_pattern, os.path.basename(site_data_db)):
                site_db_job = re.search(fortis_job_no_pattern, os.path.basename(site_data_db)).groups()[0]
                if document_job_no.upper() != site_db_job.upper():
                    raise forms.ValidationError(
                        _('Site Data db: %(site_data_db)s, has job no %(site_db_job)s'
                          ' (Different from %(document_job_no)s) in document file !!!'),
                        params={'site_data_db': os.path.basename(site_data_db),
                                'site_db_job': site_db_job, 'document_job_no': document_job_no}
                    )
        if site_data_db is not None:
            self.cleaned_data['create_gis_data'] = False
    ...
    def save(self):
        logger_request.info('Start saving in form', extra={'username': self.user.username})
        user = self.user
        new_jxl_obj = super(DataExportForm, self).save(commit=False)
        document_path = str(new_jxl_obj.document.file.name)
        logger_request.info('Document path: {}'.format(document_path), extra={'username': self.user.username})
        document_name = os.path.basename(document_path)
        if document_name[-4:].lower() == '.jxl' and os.path.isfile(document_path):
        ...
        if new_jxl_obj.site_data_db is not None:
